            ...
            match CLIENT.member_info().await {
                ...
                Err(err) => {
                    match err.info {
                        ...
                        ErrorInfo::Message(_) => {
                            // Token has expired
                            // todo : Used to log in again after token expiration
...
async fn clean_image(time: i64) -> Result<()> {
    ...
    loop {
        ...
        for cache in caches {
            ...
            image_cache::delete_by_cache_key(cache.cache_key).await?; // Regardless of how many entries are affected
            let _ = std::fs::remove_file(local); // Regardless of success
        }
    }
    Ok(())
}
...
pub fn desktop_root() -> Result<String> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let home = std::env::var_os("HOME")
            ...
            .with_context(|| "error")?
            .to_string();
        Ok(join_paths(vec![home.as_str(), ".opensource", "MangaLord"]))
    }
