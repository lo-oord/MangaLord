...
class AnimeTitle {
  const AnimeTitle({required this.id, required this.title, required this.url, required this.sourceKey, required this.sourceName, this.sourceLogo = '', this.poster = '', this.cover = '', this.background = '', this.description = '', this.genres = const <String>[], this.alternativeTitles = const <String>[], this.status = '', this.year = '', this.season = '', this.type = '', this.rating = '', this.episodes = const <AnimeEpisode>[]});
  final String id, title, url, sourceKey, sourceName, sourceLogo, poster, cover, background, description, status, year, season, type, rating;
  final List<String> genres, alternativeTitles;
  final List<AnimeEpisode> episodes;
}
class AnimeEpisode {
  const AnimeEpisode({required this.id, required this.title, required this.url, required this.sourceKey, this.number = '', this.thumbnail = '', this.addedAt, this.servers = const <AnimeServer>[]});
  final String id, title, url, sourceKey, number, thumbnail;
  final DateTime? addedAt;
  final List<AnimeServer> servers;
}
class AnimeServer {
  const AnimeServer({required this.name, required this.url, required this.sourceKey, this.quality = '', this.type = '', this.headers = const <String, String>{}, this.subtitles = const <AnimeSubtitle>[]});
  final String name, url, sourceKey, quality, type;
...
String extractMediaUrl(String value, Uri base) { final urls = extractMediaUrls(value, base); return urls.isEmpty ? '' : urls.first; }
List<String> extractMediaUrls(String value, Uri base) {
  final decoded = value.replaceAll(r'\/', '/').replaceAll(r'\u0026', '&').replaceAll('&amp;', '&');
  return RegExp(r'''(?:https?:)?//[^\s"'<>\\]+(?:\.m3u8|\.mp4)(?:\?[^\s"'<>\\]*)?''', caseSensitive: false).allMatches(decoded).map((match) => resolveSourceUrl(base, match.group(0)!)).where((url) => url.isNotEmpty).toSet().toList();
}
