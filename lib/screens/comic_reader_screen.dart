...
////////////////////////////////
// Android only
// The listener intercepts Android volume keys
// Only the last listener is effective
// event may be DOWN/UP
  ...
  void _onNextAction() {
    if (_hasNextEp()) {
      ...
      if (nextEp != null) {
        ...
        defaultToast(context, "You have reached the end");
      }
    } else {
      defaultToast(context, "You have reached the end");
    }
...
class _ComicReaderWebToonState extends _ComicReaderState {
  ...
  void _onListCurrentChange() {
    ...
    // Contains one Next chapter; assuming five images 0,1,2,3,4 length=5, Next chapter=5
  ...
  void _needJumpTo(int index, bool animation) {
    if (noAnimation() || animation == false) {
      ...
      _itemScrollController.scrollTo(
        index: index, // decrement index; current position minus one is the previous item
      ...
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        return ScrollablePositionedList.builder(
          ...
          padding: EdgeInsets.only(
            // Regardless of full-screen mode or No, keep top spacing constant for every scroll direction
            top: super._appBarHeight(),
            bottom: widget.readerDirection == ReaderDirection.topToBottom
                ? 130 // Vertical scrolling; always keep 130 pixels of bottom spacing
                : ( // Horizontal scrolling
                    widget.fullScreen
                        ? super._appBarHeight() // Keep equal spacing from the top and bottom screen edges in full-screen mode
                        : super._bottomBarHeight())
            // In non-full-screen mode, remove the top and bottom bar heights to create a filled appearance
            ,
          ),
    ...
    return Container(
      ...
      child: MaterialButton(
        ...
        child: Container(
          ...
          child: Text(super._hasNextEp() ? 'Next chapter' : 'Finish reading'),
        ),
      ),
    );
  }
