...
class Manga {
  ...
  factory Manga.fromJson(Map<String, dynamic> json) => Manga(title: json['title'] as String? ?? '', url: json['url'] as String? ?? '', author: json['author'] as String? ?? '', genre: json['genre'] as String? ?? '', cover: json['cover'] as String? ?? '', description: json['description'] as String? ?? '', chapters: (json['chapters'] as num?)?.toInt() ?? 0, status: json['status'] as String? ?? 'Ongoing', chapterItems: ((json['chapterItems'] as List?) ?? const []).whereType<Map>().map((item) => TeamXChapter.fromJson(Map<String, dynamic>.from(item))).toList());
}
...
class _AppScreenState extends State<AppScreen> {
  ...
  Future<void> _loadLatest({bool silent = false}) async {
    ...
    try {
      ...
      if (mounted) setState(() { manga = result.map(Manga.fromTeamX).toList(); loading = false; });
    } catch (e) {
