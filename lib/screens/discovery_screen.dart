...
import 'package:manga_lord/screens/comic_search_screen.dart';
...
class _DiscoveryScreenState extends State<DiscoveryScreen> {
  ...
  int _tagsLoadStatus = 0; // 0 : Loading , 1 : Loaded successfully , 2 : Loading failed
  late UITags uiTags;
    ...
    if (_tagsLoadStatus == 0) {
      ...
      return AppBar(
        title: MaterialButton(
          onPressed: _loadTags,
          child: const Text("Failed to load categories. Tap to retry"),
        ),
        actions: [
          _b,
          _searchBar.getSearchAction(context),
      ...
      final orderingButton = PopupMenuButton<String>(
        ...
        itemBuilder: (BuildContext context) {
          ...
          for (var ordering in uiTags.ordering) {
            ...
            //   child: ListTile(
            //     title: Text("${ordering.name}(Descending)"),
            //   ),
            // ));
            // orderingItems.add(PopupMenuItem<String>(
            //   value: ordering.pathWord,
            //   child: ListTile(
            //     title: Text("${ordering.name}(Ascending)"),
            //   ),
            // ));
            orderingItems.add(PopupMenuItem<String>(
              value: "-${ordering.pathWord}",
